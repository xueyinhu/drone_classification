...
class AttentionBasic(Model):
    ...
    def __init__(self, c, h, w):
        ...
        self.se = Sequential([
            BatchNormalization(),
            Conv2D(c, (3, 3), padding='same', groups=c // 4),
            BatchNormalization(),
            ELU(),
        ])

    def call(self, x):
        y = tf.concat([self.s1_m(x), self.s1_a(x)], axis=1)
        t1 = self.s1(y) * x
        y = tf.concat([self.s2_m(x), self.s2_a(x)], axis=2)
        t2 = self.s2(y) * x
        y = tf.concat([self.s3_m(x), self.s3_a(x)], axis=1)
        t3 = self.s3(y) * x
        t1 = tf.sigmoid(t1)
        t2 = tf.sigmoid(t2)
        t3 = tf.sigmoid(t3)
